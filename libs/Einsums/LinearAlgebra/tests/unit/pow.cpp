...
TEMPLATE_TEST_CASE("pow", "[linear-algebra]", float, double) {
    ...
    SECTION("Square root") {
        ...
        auto B = linear_algebra::detail::sqrt(A);

        for (int i = 0; i < A.dim(0); i++) {
            for (int j = 0; j < A.dim(1); j++) {
                REQUIRE_THAT(B(i, j), Catch::Matchers::WithinAbs(std::sqrt(A(i, j)), TestType{1e-5}));
            }
        }
    }
    ...
    SECTION("Integer power") {
        ...
        Tensor B = linear_algebra::pow(A, 2);
        ...
        for (int i = 0; i < A.dim(0); i++) {
            for (int j = 0; j < A.dim(1); j++) {
                REQUIRE_THAT(B(i, j), Catch::Matchers::WithinAbs(C(i, j), TestType{1e-5}));
            }
        }
    }

    SECTION("Integer power as non integer type") {
    ...
    SECTION("Negative Integer power") {
        ...
        Tensor<TestType, 2> Ainv = A;

        linear_algebra::invert(&Ainv);

        Tensor B = linear_algebra::pow(A, -2);
        Tensor C = create_tensor_like(A);

        linear_algebra::gemm<false, false>(1.0, Ainv, Ainv, 0.0, &C);
    ...
    SECTION("Negative Integer power as non integer type") {
    ...
    SECTION("Non-integer power") {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                REQUIRE_THAT(B(i, j), Catch::Matchers::WithinAbs(C(i, j), TestType{1e-3}));
                REQUIRE_THAT(C(j, i), Catch::Matchers::WithinAbs(C(i, j), TestType{1e-3}));
                REQUIRE_THAT(B(j, i), Catch::Matchers::WithinAbs(B(i, j), TestType{1e-3}));
            }
        }
    }

    SECTION("Weird Non-integer power") {
        constexpr int       size = 3;
        Tensor<TestType, 2> A{"A", size, size};
        Tensor<TestType, 2> B{"B", size, size};
        Tensor<TestType, 2> C{"C", size, size};
        ...
        for (int i = 0; i < size; i++) {
            Evals(i) = std::pow(Evals(i), TestType{1.0 / 3.0});
        }
        auto Diag2 = diagonal(Evals);
        ...
        C.zero();
        C = linear_algebra::pow(A, TestType{1.0 / 3.0});
    ...
    SECTION("Integer power blocks") {
        ...
        BlockTensor B = einsums::linear_algebra::pow(A, 2);
